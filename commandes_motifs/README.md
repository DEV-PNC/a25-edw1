...
1. Pourquoi `lien_physique.txt` fonctionne encore après suppression de l'original ?
2. Que montre `ls -l` pour le lien symbolique cassé ?
...
1. **Toujours tester avec `echo` ou `ls` avant `rm`, `mv`, `cp`**
2. **Utiliser `-i` (interactif) quand on débute**
3. **Les patterns ne marchent pas sur les fichiers cachés (`.`)** sans option
4. **Guillemets** pour les variables dans les boucles : `"$f"`
5. **Vérifier** avec `ls -l` après les opérations sur les liens
